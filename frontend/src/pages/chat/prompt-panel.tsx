...
const ExpandableSettingPanel = () => {
    ...
    const [epselectedOption, setepSelectedOption] = useState<SelectProps.Option>({
        value: endpointName,
        label: endpointName
    });
    ...
    const handleLoadItems = async ({
        ...
        try {
            ...
            const epInfo = response.endpoints.map((item: any) => ({
    ...
    useEffect(() => {
        ...
        setLocalStoredParams({
            ...
            use_conversation:
                localStoredParams?.use_conversation === undefined
                    ? defaultModelParams.use_conversation
                    : localStoredParams?.use_conversation,
            ...
            <ColumnLayout borders="vertical" columns={3} variant="text-grid">
                <FormField label={t("endpoint_name")} errorText={!endpointName ? "select model first" : ""}>
                    <Select
                        ...
                        options={endpointsInfo.map((item) => ({
                            label: item.endpoint_name,
...
const PromptPanel = ({sendMessage}: { sendMessage: ({id, messages, params}: MessageDataProp) => void }) => {
    ...
    const [useConversationChecked, setUseConversationChecked] = useState(
        localStoredParams?.use_conversation !== undefined
            ? localStoredParams?.use_conversation
            : defaultModelParams.use_conversation
    );
    ...
    const onSubmit = (values: string) => {
        ...
        const messages = modelParams.use_conversation ? [...conversations, inputMessage] : [inputMessage];
                ...
                <SpaceBetween size="s">
                    ...
                    <SpaceBetween size="xl" direction="horizontal">
                        <FormField>
                            ...
                            <Toggle
                                onChange={({detail}) => {
                                    setUseConversationChecked(detail.checked);
                                    setModelParams((prev: MsgItemProps[]) => ({
                                        ...prev,
                                        use_conversation: detail.checked,
                                    }));
                                    setLocalStoredParams({
                                        ...localStoredParams,
                                        use_conversation: detail.checked,
                                ...
                                checked={useConversationChecked}
                            >{t("use_history_conversation")}</Toggle>
                        </FormField>
                    </SpaceBetween>
                </SpaceBetween>
