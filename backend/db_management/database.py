...
class DatabaseWrapper(BaseModel):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.connection_pool = mysql.connector.pooling.MySQLConnectionPool(pool_name="mypool", pool_size=5,
                                                                           **MYSQL_CONFIG)
    ...
    def update_endpoint_status(self,
        ...
        with self.connection_pool.get_connection() as connection:
            with connection.cursor() as cursor:
                cursor.execute(
                    f"UPDATE {EP_TABLE} SET endpoint_status = %s,endpoint_delete_time = %s, extra_config = %s WHERE endpoint_name = %s",
            ...
            with self.connection_pool.get_connection() as connection:
                with connection.cursor() as cursor:
                    cursor.execute(
                        ...
                        (job_id, endpoint_name, model_name, engine, enable_lora, instance_type, instance_count,
                         model_s3_path, endpoint_status.value, endpoint_create_time, endpoint_delete_time,
        ...
        with self.connection_pool.get_connection() as connection:
            with connection.cursor() as cursor:
                if query_terms:
                    ...
                    cursor.execute(f"SELECT * FROM {EP_TABLE} WHERE endpoint_status <> 'TERMINATED' LIMIT %s OFFSET %s",
                                   (page_size, offset))
                return cursor.fetchall()
    ...
    def add_user(self, username: str, password: str, groupname: str,
                 extra_config={"create_time": datetime.now().strftime('%Y-%m-%d %H:%M:%S')}):
        with self.connection_pool.get_connection() as connection:
            with connection.cursor() as cursor:
                cursor.execute(
