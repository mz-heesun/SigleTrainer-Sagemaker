...
async def get_job_by_id(request: GetJobsRequest) -> JobsResponse:
    ...
    job_info = None
    if results:
        _, job_id, job_name, job_run_name, output_s3_path, job_type, job_status, job_create_time, job_start_time, job_end_time, job_payload, ts = \
        results[0]
...
async def delete_job_by_id(request: DelJobsRequest) -> CommonResponse:
    ...
    return CommonResponse(response_id=str(uuid.uuid4()), response={"code": "SUCCESS" if ret else "FAILED",
                                                                   "message": "" if ret else "Job already started"})


async def list_jobs(request: ListJobsRequest) -> ListJobsResponse:
    results = database.list_jobs(query_terms=request.query_terms, page_size=request.page_size,
...
async def fetch_training_log(request: FetchLogRequest):
    ...
    if job_run_name:
        logs, next_forward_token, next_backward_token = fetch_log(log_stream_name=job_run_name,
                                                                  next_token=request.next_token)
    else:
        logs, next_forward_token, next_backward_token = [
            'No logs exists, SageMaker Training Job might not exist'], None, None
