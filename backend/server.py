...
async def handle_deploy_endpoint(request: DeployModelRequest):
    ...
    if request.engine in ['auto', 'vllm']:
        try:
            ...
            return CommonResponse(response_id=str(uuid.uuid4()),
                                  response={"result": True, "endpoint_name": "Too long,swithing to background process"})
    else:
        try:
            ret, msg = await asyncio.wait_for(
            ...
            return CommonResponse(response_id=str(uuid.uuid4()),
                                  response={"result": False, "endpoint_name": "Operation timed out"})
...
async def handle_inference(request: InferenceRequest):
    ...
    if engine in ['auto', 'vllm']:
        if not request.stream:
            response = inference_byoc(request.endpoint_name, request.model_name, request.messages, request.params,
                                      False)
