...
from utils.config import boto_sess, role, sagemaker_session, DEFAULT_REGION, SUPPORTED_MODELS_FILE, \
    DEFAULT_TEMPLATE_FILE, default_bucket, VLLM_IMAGE, MODEL_ARTIFACT, instance_gpus_map
...
def deploy_endpoint_byoc(job_id: str, engine: str, instance_type: str, quantize: str, enable_lora: bool,
...
Dict[bool, str]:
    ...
    try:
        print(f'lmi_image_uri:{lmi_image_uri}')
...
def deploy_endpoint_background(job_id: str, engine: str, instance_type: str, quantize: str, enable_lora: bool,
                               model_name: str, cust_repo_type: str, cust_repo_addr: str, extra_params: Dict[str, Any]):
...
def deploy_endpoint_ms(job_id: str, engine: str, instance_type: str, quantize: str, enable_lora: bool, model_name: str,
                       cust_repo_type: str, cust_repo_addr: str, extra_params: Dict[str, Any]) -> Dict[bool, str]:
    pure_model_name = model_name.split('/')[1]
    create_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    endpoint_name = sagemaker.utils.name_from_base(pure_model_name).replace('.', '-').replace('_', '-')
    instance_count = int(extra_params.get("instance_count", 1))
    ...
    ms_download_and_upload_model(model_repo=model_name, s3_bucket=default_bucket,
                                 s3_prefix=f"original_model_file/{model_name}")
    logger.info(f"deploy endpoint with model_name:{model_name},model_path:{model_path}")
...
# lmi-dist를 사용하는 경우 trtllm 엔진은 이 기능을 사용합니다.
def deploy_endpoint(job_id: str, engine: str, instance_type: str, quantize: str, enable_lora: bool, model_name: str,
                    cust_repo_type: str, cust_repo_addr: str, extra_params: Dict[str, Any]) -> Dict[bool, str]:
    ...
    # 원래 모델을 사용하는 경우
        ...
        if repo_type == DownloadSource.DEFAULT:
            ...
            # 모델 범위인 경우 로컬로 다운로드해야 합니다.
            model_path = ms_download_and_upload_model(model_repo=model_name, s3_bucket=default_bucket,
                                                      s3_prefix=f"original_model_file/{model_name}")
    # 如果是使用自定义模型
    elif not cust_repo_addr == '' and model_name == '':
        model_name = cust_repo_addr
        ...
        if repo_type == DownloadSource.DEFAULT:
            ...
            # 如果是模型scope，则需要下载到本地
            model_path = ms_download_and_upload_model(model_repo=cust_repo_addr, s3_bucket=default_bucket,
                                                      s3_prefix=f"original_model_file/{model_name}")
...
def list_endpoints(request: ListEndpointsRequest) -> Dict[EndpointInfo, int]:
    ...
    results = database.list_endpoints(query_terms=request.query_terms, page_size=request.page_size,
    ...
    info = [EndpointInfo(job_id=job_id,
            ...
            for
            _, job_id, endpoint_name, model_name, engine, enable_lora, instance_type, instance_count, model_s3_path, endpoint_status, endpoint_create_time, endpoint_delete_time, extra_config
            in results]
